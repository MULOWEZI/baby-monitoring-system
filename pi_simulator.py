...
# ============================================================
#
# Alerts are EVENT based, not LEVEL based.
#
# Temperature:
#   normal -> abnormal = NEW ALERT
#   abnormal -> abnormal = NO NEW ALERT
#   abnormal -> normal = RESET
#   normal -> abnormal = NEW ALERT again
#
# Wetness:
#   dry -> wet = NEW ALERT
#   wet -> wet = NO NEW ALERT
#   wet -> dry = RESET
#   dry -> wet = NEW ALERT again
#
# The state is kept in memory. With the recommended Render
# deployment of ONE worker, this state remains consistent.
# ============================================================
...
# Optional: keep the latest alert states visible to /health/debugging.
alert_state = {
    "temperature_abnormal": False,
    "wetness_active": False,
    "last_event": None,
}
...
def check_alerts(temp, hum, wetness, sound):
    """
    Detect and process NEW alert events.

    IMPORTANT:
    This function does NOT send an alert every time a sensor
    reading is abnormal.
    ...
    Wetness works the same way:
        dry -> wet = alert
        wet -> wet = nothing
        wet -> dry = reset
        dry -> wet = alert again

    Returns the list of NEW alerts generated during this reading.
    """
    ...
    if temp is not None:
        try:
            ...
            log.warning(
                "Invalid temperature for alert check: %r",
                temp,
            )
    ...
    # ------------------------------------------------------------
    # Detect state transitions atomically
    # ------------------------------------------------------------
    ...
    with alert_state_lock:
        ...
        # IMPORTANT:
        # Always save the current state so the next reading knows
        # whether this is a new event or the same ongoing event.
        ...
        # Reset automatically when diaper becomes dry.
        previous_wetness = current_wetness

        # Update debug state.
        alert_state["temperature_abnormal"] = (
            temperature_abnormal
        )

        alert_state["wetness_active"] = (
            current_wetness
        )

        if alerts:
            alert_state["last_event"] = (
    ...
    # ------------------------------------------------------------
    #
    # This is intentionally NOT inside "if supabase is not None".
    # The browser should receive the alert even if the database
    ...
    # ------------------------------------------------------------
    #
    # Database writes happen in a background thread so a slow
    # Supabase request cannot delay the Raspberry Pi.
    # ------------------------------------------------------------

    if supabase is not None:

        def save_alert_events(new_alerts):
            ...
            for alert in new_alerts:
                ...
                try:
                    ...
                    log.error(
                        "Supabase alert insert failed: %s",
                        exc,
                    )

        threading.Thread(
            target=save_alert_events,
            args=(list(alerts),),
            daemon=True,
            name="alert-db",
        ).start()
    ...
    # ------------------------------------------------------------
    # ONE EMAIL FOR THIS NEW EVENT
    # ------------------------------------------------------------
    #
    # If temperature and wetness become abnormal in the same
    # sensor reading, send_alert_email() receives both alerts and
    # sends ONE email containing both.
    #
    # It is called ONLY because "alerts" contains NEW events.
    # Ongoing abnormal readings never reach this function.
    # ------------------------------------------------------------
...
def api_ingest():
    ...
    # --------------------------------------------------------
    # Convert incoming values to correct types.
    # This prevents strings such as "true" or "26.5" from
    # breaking alert comparisons.
    # --------------------------------------------------------
...
def api_alert_state():
    ...
    with alert_state_lock:
        ...
        state["previous_temperature_abnormal"] = (
            previous_temperature_abnormal
        )
        state["previous_wetness"] = (
            previous_wetness
        )
...
def api_test_alert():
    ...
    This is useful for testing Socket.IO, Supabase and email
    without changing the Raspberry Pi sensors.
    """
    ...
    if alert_type in ("temperature", "both"):
    ...
    if not test_alerts:
        return jsonify({
            "error":
                "type must be temperature, wetness or both"
        }), 400
        ...
        def save_test_alerts():
            ...
            for alert in test_alerts:
                ...
                try:
                    ...
                    log.error(
                        "Test alert DB error: %s",
                        exc,
                    )

        threading.Thread(
            target=save_test_alerts,
            daemon=True,
            name="test-alert-db",
        ).start()
